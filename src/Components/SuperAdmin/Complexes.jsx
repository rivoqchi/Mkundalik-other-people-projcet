...
function Departamentlar() {
  ...
  const handleDeleteSection = async () => {
    try {
      await axios.delete(`${API}/complexes/deletecomplexbyid/${selectedSection._id}`);
      setAlert({
        show: true,
        type: "success",
  ...
  return (
    <div className={`p-4 ${isDark ? "bg-slate-900" : "bg-light"}`} style={{ minHeight: "100vh", transition: "all 0.3s ease" }}>
      ...
      <div className="d-flex flex-wrap justify-content-between align-items-center mb-5 mt-3 px-3">
        <div>
          <h2 className={`fw-bold ${isDark ? "text-white" : "text-dark"}`}>Komplekslar</h2>
          <p className={`mb-0 ${isDark ? "text-white-50" : "text-muted"}`}>Jami: {allSections.length} ta kompleks mavjud</p>
        </div>
        <Link to="/superadmin/complex/add" className="text-decoration-none mt-3 mt-md-0">
          <Button variant="primary" className="rounded-pill px-4 shadow-sm fw-bold">
            <i className="fa-solid fa-plus me-2"></i> Yangi kompleks
          </Button>
        </Link>
      </div>
          ...
          {allSections.map((section) => (
            <div className="col-12 col-md-6 mb-4" key={section._id}>
              ...
              >
                <div className={`p-4 d-flex justify-content-between align-items-center border-bottom ${isDark ? 'border-light border-opacity-10' : 'border-dark border-opacity-10'}`}>
                  <div className="d-flex align-items-center gap-3">
                    <div className="rounded-circle bg-primary bg-opacity-10 p-3 d-flex align-items-center justify-content-center" style={{ width: '50px', height: '50px' }}>
                      <i className="fa-solid fa-sitemap text-primary fs-4"></i>
                    </div>
                    <h5 className={`mb-0 fw-bold ${isDark ? 'text-light' : 'text-dark'}`}>{section.name}</h5>
                  </div>
                  <div className="d-flex gap-2">
                    <Link to={`/superadmin/complex/edit/${section._id}`} className="btn btn-outline-primary btn-sm rounded-circle" title="Tahrirlash">
                ...
                <div className={`p-3 d-flex flex-wrap gap-2 justify-content-center ${isDark ? 'bg-dark bg-opacity-25' : 'bg-light bg-opacity-50'}`}>
                  <button
                    className={`btn btn-sm ${isDark ? 'btn-outline-light' : 'btn-outline-dark'} rounded-pill px-4 py-1 fw-medium`}
                    onClick={() => handleShowAdmins(section)}
                  >
                    <i className="fa-regular fa-building text-info me-2"></i>
                    {section.departments.length} ta departament
                  </button>
                </div>
              </div>
            </div>
      ...
      {selectedSection && (
        <Modal show={show} onHide={handleClose} centered>
          ...
          <Modal.Body className={isDark ? "bg-dark text-white" : "bg-white text-dark"}>
            Rostdan ham <strong>{selectedSection.name}</strong> kompleksini o‘chirmoqchimisiz?
          </Modal.Body>
      ...
      {/* Admins/Departments Modal */}
      {showAdmins && selectedSection && (
        <Modal show={showAdmins} onHide={handleCloseAdmins} centered>
          <Modal.Header closeButton className={isDark ? "bg-dark text-white border-secondary" : "bg-white text-dark border-bottom"}>
            <Modal.Title className="fs-5">{selectedSection.name} departamentlari</Modal.Title>
          </Modal.Header>
          <Modal.Body className={isDark ? "bg-dark text-white p-2" : "bg-white text-dark p-2"}>
            <div className="list-group list-group-flush" style={{ maxHeight: "400px", overflowY: "auto" }}>
              {selectedSection.departments.map((admin) => (
                <div
                  key={admin._id}
                ...
                >
                  <div className="rounded-circle bg-info bg-opacity-25 p-2 d-flex align-items-center justify-content-center">
                    <i className="fa-regular fa-building text-info"></i>
                  </div>
                  <span className="fw-medium">{admin.name}</span>
                </div>
              ))}
              {selectedSection.departments.length === 0 && (
                <div className={`text-center p-4 ${isDark ? 'text-white-50' : 'text-muted'}`}>Ushbu kompleksga departamentlar biriktirilmagan</div>
              )}
            </div>
          </Modal.Body>
          <Modal.Footer className={isDark ? "bg-dark text-white border-secondary" : "bg-white text-dark border-top"}>
