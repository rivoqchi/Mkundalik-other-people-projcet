            ...
            <div className="list-group list-group-flush" style={{ maxHeight: "400px", overflowY: "auto" }}>
              {selectedSection.admins.map((admin) => (
                <Link
                  ...
                  className={`list-group-item list-group-item-action border-0 mb-1 rounded d-flex align-items-center gap-3 ${isDark ? 'bg-warning bg-opacity-10 text-light hover-bg-secondary' : 'bg-light bg-opacity-50 text-dark hover-bg-light'}`}
                >
              ...
              {selectedSection.admins.length === 0 && (
                <div className={`text-center p-4 ${isDark ? 'text-white-50' : 'text-muted'}`}>Bo'lim boshlig'i biriktirilmagan</div>
              )}
            </div>
          </Modal.Body>
          <Modal.Footer className={isDark ? "bg-dark text-white border-secondary" : "bg-white text-dark border-top"}>
